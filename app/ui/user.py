...
def create_user(req: Request, user: UserCreate, session: Session = Depends(get_session)):
...
def test(req: Request, session: Session = Depends(get_session)):
    user_repo = UserRepository(session)
...
def get_own_items(req: Request, db: Session = Depends(get_session), current_user: User = Depends(get_current_active_user)):
...
def create_own_item(req: Request, item: ItemCreate, db: Session = Depends(get_session), current_user: User = Depends(get_current_active_user)):
    item_repo = ItemRepository(db)
