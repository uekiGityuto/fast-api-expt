...
def get_users(req: Request, skip: int = 0, limit: int = 100, db: Session = Depends(get_session)):
...
def get_user(req: Request, user_id: int, db: Session = Depends(get_session)):
    user_repo = UserRepository(db)
...
def delete_user(req: Request, user_id: int, db: Session = Depends(get_session)):
    user_repo = UserRepository(db)
...
def create_item_for_user(req: Request, user_id: int, item: ItemCreate, db: Session = Depends(get_session)):
...
def get_items(req: Request, skip: int = 0, limit: int = 100, db: Session = Depends(get_session)):
    item_repo = ItemRepository(db)
